...
| 📚 Wiki weekly wrap-up ⏸    | 11pm Sunday      | container | **Paused.** Lint pass, dedup candidates, cluster promotion from `unorganized.md`, writes `last-run-manifest.md` (Opus)                                                                 |
| 📚 Wiki folder review ⏸     | 3am 1st of month | container | **Paused.** Evaluates wiki folder organization, writes scannability-focused proposal to `resources/wiki/folder-review.md` (proposal-only; apply happens via Claude Code session)       |
| 🗄 Archive Briefings   | Midnight Sunday  | script    | Moves old briefing files to `_archive/` per retention rules                                                                                                          |
| Vault Sync             | Every 30 min (`*/30`) | script | Commits and pushes vault changes to GitHub                                                                                                                        |
| Dotfiles Sync          | Every 30 min (`5,35`) | script | Commits, rebases, and pushes `~/dotfiles`. Backstop for the Claude Code session hooks — a session left running for days never fires SessionEnd. Offset from Vault Sync so the logs stay readable. |

**These are NanoClaw tasks, not cron.** The schedule lives in the `scheduled_tasks` table in `~/nanoclaw/store/messages.db` — nothing appears in `crontab -l` or `launchctl`. If NanoClaw is down or wedged, every job here stops silently; see the [[2026-08-06-vault-sync-dead-mans-switch]] proposal.

**Creating a `script`-type task requires direct SQL.** The only programmatic path (`createTask()` in `src/ipc.ts`, which Telegram requests also route through) clamps `context_mode` to `group` or `isolated` and silently falls back to `isolated` — so asking NanoClaw to schedule a shell script produces a container agent burning tokens every run instead. Insert the row by hand, then update the `data/ipc/<group>/current_tasks.json` mirror per the two-sources-of-truth rule in `~/nanoclaw/CLAUDE.md`. No restart needed: `getDueTasks()` re-queries the DB every poll.

**Correction (2026-08-06):** an "Obsidian Sync Check — every 30 min" row was listed here but no such task has ever existed in `scheduled_tasks`. The script `~/nanoclaw/scripts/check-obsidian-sync.sh` exists but is not scheduled by anything. Obsidian Sync itself runs continuously as the `com.obsidian-sync` launchd agent (`ob sync --continuous`), which is unrelated to git.
