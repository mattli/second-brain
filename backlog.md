...
- [ ] Schedule category-level product research via Last 30 Days Reddit — weekly NanoClaw task (e.g. Friday or Saturday) that runs Reddit searches across a set of product categories (dev tools, productivity, AI, etc.) and saves findings to the vault. Goal: passive product signal gathering that runs on subscription tokens during off-peak hours, feeding into Sunday weekly summary or standalone product ideas doc.
