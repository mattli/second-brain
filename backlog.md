...
- [ ] Reader-facing weekly wiki digest — current Sunday wrap-up produces `last-run-manifest.md` which is a stats summary (pages created, dedup candidates, lint findings) — useful for pipeline health, useless for reading. Add a separate reader-facing digest: "What's new in the wiki — week of YYYY-MM-DD" with ~5 bullets covering meaningful additions/expansions and which pages are worth revisiting. Deliver to Telegram main on Sunday evening alongside (or via) the weekly summary. Scratches "I don't know what's new vs old in each article" without per-page changelog clutter. Trigger condition: wiki ≥50 pages OR a week with ≥5 worker commits — until then the volume is small enough that nothing's news.
