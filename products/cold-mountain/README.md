...
- /wiki navigation: sticky left sidebar on every /wiki route — section anchors on the index (`WikiNav`, `#slug` in-page) and a per-article H2 TOC on articles (`WikiToc`). Scrollspy via passive-scroll offset walk with a 160px trigger line (no IntersectionObserver). H2 IDs come from a shared `slugifyHeading()` used both by `extractH2s()` in `_lib/wiki.ts` and by an `h2` override in `markdown.tsx`, so the TOC list and the DOM IDs always agree. (As of 2026-06-30, `markdown.tsx` also assigns the same slug IDs to **H3 and H4** — PR #2 — so deep anchor links into subsections resolve; the TOC itself stays H2-only. Note: heading IDs are applied at client render, so `curl` of the raw HTML won't show them — verify deep links in a browser.) Back button (`WikiBackButton`) walks a sessionStorage path stack (`coldmountain:wikiStack`) so chains of any depth navigate cleanly: A → B → C → Back → B → Back → A → Back → /wiki → Back → /. Hash changes never touch the stack.
...
- **Obsidian cross-surface anchors (deferred 2026-06-30)** — wiki `#anchor` links use GFM kebab-slugs (`#five-building-blocks`) that the site resolves but Obsidian (Matt's primary reading surface) does not — Obsidian wants literal-heading-text anchors. No single syntax works in both. A real fix means authoring Obsidian-native anchors + a slugify transform on the site + changing the `readwise-wiki.md` `## Recent Updates` link convention. Judged not worth it (papercut-level Obsidian annoyance). Revisit only if it starts nagging in daily reading. Full analysis in agent memory `wiki-dual-surface-anchors.md`.
