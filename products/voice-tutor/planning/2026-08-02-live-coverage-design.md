...
> **Read path wired 2026-08-06** (branch `feat/coverage-read-path`, unmerged, awaiting review). The
> two carried items below are closed: `union_for_document` now degrades per-sidecar instead of raising,
> and coverage reaches four surfaces — the picker, the pre-connect screen, the freshly-ended view, and a
> past session, which shows the union AS OF that session rather than today's total. `backfill_coverage.py`
> still writes no ledger row. Decision 3 (live display) remains Phase 2. Timestamp-format inconsistency
> found while reporting a judged_at value: [[2026-08-06-timestamp-format-consistency]].
