...
- **#5 (preview) Click-to-explain (✅ shipped 2026-04-30, narrow form)** — Stripped-down version of the full menu: single fixed action ("explain like I'm five"), click-driven, markdown-only. Click any paragraph in a markdown document → right-side panel slides in with a Sonnet 4.6 explanation grounded in the full document. Panel shows just the explanation (the highlighted paragraph in the doc is the anchor — no re-paste). Persistent highlight after dismiss as a memory cue. In-memory per-paragraph cache so re-clicking is instant and doesn't re-pay. Body shifts left when panel opens (no overlay). Manual close only (Escape / X). Source-only grounding lives in the prompt (`mvp-scope.md` says hallucination is a dealbreaker for the validated user; v1 prompt explicitly tells the model not to extrapolate beyond the document). Two files end-to-end: `src/app/api/explain/route.ts` + `src/components/reader/ExplainPanel.tsx`. PDF returns 400; PDF sentence-click is the unrelated unresolved problem above. Out of scope (do NOT extend without scoping): multi-action menu (define / examples / etc.), chat / follow-up, session state, persistence, streaming, RAG / chunking. The full feature #5 unfolds from this once the click model is validated.

**V1 remaining**: PDF sentence-click decision (improve matching vs. native selection), full multi-action menu + LLM integration (#5–6 proper), chat (#7), session state + artifact (#8), vector storage + RAG with grounding constraint (#9), source-type metadata (#10 — woven throughout), progress tracking (#11). See `mvp-scope.md` for build order.
