...
- [ ] **Word-level definitions via double-click.** Single-click is reserved for paragraph-level explanation. Double-click on a word triggers a definition lookup — separate gesture, separate visual treatment (small inline tooltip near the word, not the right-side panel). Avoids click-target ambiguity (single-click belongs to the paragraph it's inside). Definition can be document-grounded first (does the document define this term?) with a fallback to general knowledge if not. Fits the existing interaction model without adding a new mode. Decide implementation details when feature #5 (action menu) ships — might also surface "define key terms" as a follow-up action on a paragraph for the case where the user wants definitions for multiple terms at once.
